...
/**
 * GET /api/chesscom/:username/progress
 * Get user progress statistics from Chess.com
 */
router.get('/:username/progress', async (req, res) => {
  try {
    ...
    console.log(`Fetching Chess.com progress statistics for: ${username}`);

    // Fetch progress statistics from Chess.com
    const progressStats = await chesscomService.getUserProgressStats(username);
    ...
    res.status(500).json({
      error: 'Internal server error',
      message: 'Failed to fetch progress statistics from Chess.com'
    });
  }
});

...
const ImprovedStockfishGame = () => {
  ...
  const getComputerMove = async (fen) => {
    try {
      ...
      setTimeout(() => {
        if (onMoveRef.current.makeAIMove && data.bestMove) {
          // Update evaluation
          setGameState(prev => ({
            ...prev,
          ...
          const moveResult = onMoveRef.current.makeAIMove(data.bestMove);
          ...
          if (moveResult) {
            ...
            setGameState(prev => ({
              ...prev,
              fen: onMoveRef.current.getCurrentPosition(),
              isPlayerTurn: true
            }));
            
            // Check for game end
            const gameObj = onMoveRef.current.getGameObject();
